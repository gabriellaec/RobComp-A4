...
if __name__=="__main__":
	...
	try:
		...
		while not rospy.is_shutdown():
			...
			if len(media) != 0 and len(centro) != 0:
				...
				if (media[0] == centro[0]):# and acabou == False:
					vel = Twist(Vector3(0.6,0,0), Vector3(0,0,0.0))
					centro_creeper = True
					final=True
					print("blaaaaaaaaaaa")
					#rospy.sleep(.5)
		...
		#	identifica_creeper(cv_image)
			...
			p=len(dist)-1
			#if (media[0] > centro[0]):
			#			vel = Twist(Vector3(0,0,0.12), Vector3(0,0,-0.03))
			#if (media[0] < centro[0]):
						#vel = Twist(Vector3(0,0,0.12), Vector3(0,0,0.03))
			...
			if p>0:
				...
				if dist[p] <= 3 and dist >=2.8:# and acabou == False:

					print("oiiiiiiiii")
				 	if (media[0] > (centro[0] and final)):
						vel = Twist(Vector3(0.15,0,0), Vector3(0,0,-0.01))
					if (media[0] < (centro[0]) and final):
						vel = Twist(Vector3(0.15,0,0), Vector3(0,0,0.01))

					
				
				if dist[p] <= 1.5 and dist[p] >= 1.3 and final:
					print("oiiiiiiiii")
					...
					anda = Twist(Vector3(0.3, 0, 0), Vector3(0, 0, 0))
